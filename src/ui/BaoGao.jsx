...
export function BaoGao({ report }) {
  ...
  useEffect(() => {
    ...
    b.setOption({
      ...
      legend: { data: ['实际数量', '基准值'], textStyle: { color: AXIS }, top: 0, itemWidth: 12, itemHeight: 8 },
      grid: { left: 36, right: 12, top: 30, bottom: 24 },
      xAxis: { type: 'category', data: names, axisLabel: { color: AXIS, fontSize: 11 }, axisLine: { lineStyle: { color: LINE } } },
      yAxis: {
        type: 'value',
      ...
      series: [
        { name: '实际数量', type: 'bar', barWidth: 12, data: fps.map((x) => x.shuliang), itemStyle: { color: '#2f9bff', borderRadius: [4, 4, 0, 0] } },
        { name: '基准值', type: 'bar', barWidth: 12, data: fps.map((x) => BIAOZHUN[x.fenlei] || 3), itemStyle: { color: '#c6d8ee', borderRadius: [4, 4, 0, 0] } },
      ],
    });
